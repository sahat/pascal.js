...
function Parser(scanner) {
  ...
  this.parseRelationExpression = function(symbolTable) {
    ...
    while (true) {
      var token = this.scanner.lookAhead();
      if (token.isSymbol('=')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.EQUALITY,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else if (token.isSymbol('<>')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.INEQUALITY,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else if (token.isSymbol('>')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.GREATER_THAN,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else if (token.isSymbol('<')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.LESS_THAN,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else if (token.isSymbol('>=')) {
        node = this.createBinaryOperationNode(symbolTable, token, node,
          Node.GREATER_OR_EQUAL_TO,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else if (token.isSymbol('<=')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.LESS_OR_EQUAL_TO,
          this.parseAddExpression).withExpressionType(Node.booleanType);
      } else {
        break;
      }
  ...
  this.parseAddExpression = function(symbolTable) {
    ...
    while (true) {
      var token = this.scanner.lookAhead();
      if (token.isSymbol('+')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.ADDITION,
          this.parseMultiplyExpression);
      } else if (token.isSymbol('-')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.SUBTRACTION,
          this.parseMultiplyExpression);
      } else if (token.isReserved('or')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.OR,
          this.parseMultiplyExpression,
          Node.booleanType);
      } else {
        break;
  ...
  this.parseMultiplyExpression = function(symbolTable) {
    var node = this.parseUnaryOperatorExpression(symbolTable);
    while (true) {
      var token = this.scanner.lookAhead();
      if (token.isSymbol('*')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.MULTIPLICATION,
          this.parseUnaryOperatorExpression);
      } else if (token.isReserved('div')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.INTEGER_DIVISION,
          this.parseUnaryOperatorExpression, Node.integerType);
      } else if (token.isReserved('mod')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.MOD,
          this.parseUnaryOperatorExpression, Node.integerType);
      } else if (token.isReserved('and')) {
        node = this.createBinaryOperationNode(symbolTable, token, node, Node.AND,
          this.parseUnaryOperatorExpression, Node.booleanType);
      } else {
        break;
      }
    }
  ...
  this.parseUnaryOperatorExpression = function(symbolTable) {
    var node;
    var token = this.scanner.lookAhead();
    if (token.isSymbol('-')) {
      var expression = this.parseUnaryOperatorExpression(symbolTable);
      node = new Node(Node.NEGATIVE, token, {
        expression: expression
      });
    } else if (token.isSymbol('+')) {
      node = this.parseUnaryOperatorExpression(symbolTable);
  ...
  this.createBinaryOperationNode = function(symbolTable, token, node, nodeType, rhsFn, forceType) {
