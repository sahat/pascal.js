...
var Compiler = function() {
    ...
    switch (node.nodeType) {
      ...
      case Node.PROGRAM:
        ...
        for (var j = 0; j < ujpAddresses.length; j++) {
          this.setOperand2(ujpAddresses[j], returnAddress);
        }
        break;
      case Node.BLOCK:
      ...
      case Node.PROCEDURE_CALL:
        ...
        for (var i = 0; i < node.argumentList.length; i++) {
          ...
          if (argument.byReference) {
            this._generateAddressBytecode(argument, symbolTable);
          } else {
        ...
        if (symbol.isNative) {
          ...
          this.add(opcodes.CSP, node.argumentList.length, index);
        } else {
          var parameterSize = symbol.type.getTotalParameterSize();
          this.add(opcodes.CUP, parameterSize, symbol.address);
        }
        break;
      case Node.FOR:
        var varNode = node.variable;
        this._generateAddressBytecode(varNode, symbolTable);
        ...
        this.setOperand2(jumpInstruction, endOfLoop);
        break;
      case Node.IF:
        ...
        this.setOperand2(skipThenInstruction, falseAddress);
        if (hasElse !== -1) {
          var endOfIf = this.getNextAddress();
          this.setOperand2(skipElseInstruction, endOfIf);
        }
        break;
      case Node.NOT:
        this.generateBytecode(node.expression, symbolTable);
      ...
      case Node.NEGATIVE:
        ...
        break;
      case Node.ADDITION:
        this._generateNumericBinaryBytecode(node, symbolTable, opcodes.ADD);
        break;
      case Node.SUBTRACTION:
        this._generateNumericBinaryBytecode(node, symbolTable, opcodes.SUB);
        break;
      case Node.MULTIPLICATION:
        this._generateNumericBinaryBytecode(node, symbolTable, opcodes.MUL);
        break;
      case Node.INTEGER_DIVISION:
        this._generateNumericBinaryBytecode(node, symbolTable, opcodes.DIV);
        break;
      case Node.MOD:
        this._generateNumericBinaryBytecode(node, symbolTable, opcodes.MOD);
        break;
      case Node.EQUALITY:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.EQ);
        break;
      case Node.INEQUALITY:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.NEQ);
        break;
      case Node.LESS_THAN:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.LT);
        break;
      case Node.GREATER_THAN:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.GT);
        break;
      case Node.LESS_OR_EQUAL_TO:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.LTE);
        break;
      case Node.GREATER_OR_EQUAL_TO:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.GTE);
        break;
      case Node.AND:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.AND);
        break;
      case Node.OR:
        this._generateComparisonBinaryBytecode(node, symbolTable, opcodes.OR);
        break;
    }
  };

  this._generateNumericBinaryBytecode = function(node, symbolTable, integerOpcode) {
  ...
  this._generateComparisonBinaryBytecode = function(node, symbolTable, opcode) {
    this.generateBytecode(node.lhs, symbolTable);
    this.generateBytecode(node.rhs, symbolTable);
  ...
  this._generateAddressBytecode = function(node, symbolTable) {
    var symbolLookup = node.symbolLookup;
    var i = opcodes.LDA;
