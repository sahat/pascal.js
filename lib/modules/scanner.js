...
var Scanner = function(input) {
  ...
  this.scanOneToken = function() {
    ...
    while (Utilities.isWhitespace(character)) {
     ...
     * Reserved Token
     */
    if (Utilities.isValidIdentifierStart(character) && !token) {
      value = '';
      while (true) {
        value += character;
        character = this.nextCharacterLookAhead();
        if (character === -1 || !Utilities.isValidIdentifierPart(character)) {
          break;
        }
        this.nextCharacter();
      }
     ...
     * Number Token
     */
    if (!token && (Utilities.isDigit(character) || character === '.')) {
      var nextCharacter;
      if (character === '.') {
        ...
        } else if (!Utilities.isDigit(nextCharacter)) {
          token = new Token('.', Token.TK_SYMBOL);
        }
      }
      if (!token) {
        ...
        while (true) {
          ...
          } else if (!Utilities.isDigit(character)) break;
          this.nextCharacter();
        }
