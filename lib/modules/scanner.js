...
var Scanner = function(input) {
  ...
  this.isAlphanumeric = function(char) {
  ...
  this.isValidIdentifierStart = function(char) {
    return this.isAlphanumeric(char) || char == '_';
  };

  this.isValidIdentifierPart = function(char) {
  ...
  this.scanOneToken = function() {
    ...
    if (this.isValidIdentifierStart(character) && !token) {
      value = '';
      while (true) {
        value += character;
        character = this.nextCharacterLookAhead();
        if (character === -1 || !this.isValidIdentifierPart(character)) {
          break;
        }
        this.nextCharacter();
      }
    ...
    if (!token && (this.isDigit(character) || character === '.')) {
      var nextCharacter;
      if (character === '.') {
        ...
        } else if (!this.isDigit(nextCharacter)) {
          token = new Token('.', Token.TK_SYMBOL);
        }
      }
      if (!token) {
        ...
        while (true) {
          ...
          } else if (!this.isDigit(character)) break;
          this.nextCharacter();
        }
